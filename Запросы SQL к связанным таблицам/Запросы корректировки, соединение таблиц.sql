...
-- удаляю жанры, которые представлены менее чем 4 книгами
DELETE FROM genre
WHERE genre_id IN (
    -- выбираю ID жанров, которые представлены менее чем 4 книгами
    SELECT genre_id
    FROM book
    GROUP BY genre_id
...
-- удаляю авторов, которые пишут в жанре "Поэзия"
DELETE FROM author
-- связываю таблицы с авторами, книгами и жанрами между собой
USING author
      INNER JOIN book
      USING(author_id)
      INNER JOIN genre
      USING(genre_id)
-- выбираю для удаления только те записи,
-- которые имеют жанр "Поэзия"
WHERE name_genre = "Поэзия";

-- SELECT * FROM author;
-- SELECT * FROM book;

-- рисую Имперского штурмовика
SELECT
